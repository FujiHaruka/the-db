...
const {SchemaResource, LogResource} = require('./resources')
...
class TheDB extends TheDBBase {
  constructor (config = {}) {
    ...
    this.loadFromMapping({
      TheDBSchema: SchemaResource,
      TheDBLog: LogResource,
      ...resources
    })
  }
  ...
  load (ResourceClass, resourceName) {
    unlessProduction(({ok}) => {
      ...
      const isResource = (ResourceClass.prototype instanceof Resource) || (ResourceClass === Resource)
      ok(isResource, 'ResourceClass should inherit TheDB.Resource')
    })
    ...
    if (onCreate) {
      resource.listenToCreate(onCreate)
    }
    if (onUpdate) {
      resource.listenToUpdate(onUpdate)
    }
    if (onDestroy) {
      resource.listenToDestroy(onDestroy)
    }
    if (onDrop) {
      resource.listenToDrop(onDrop)
    }
...
module.exports = Object.assign(TheDB, {
  Resource,
  Refresher,
  DataTypes,
})
