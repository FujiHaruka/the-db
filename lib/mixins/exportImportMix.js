    ...
    async import (dirname) {
      ...
      for (const [rName, rInfo] of Object.entries(resourcesInfo)) {
        ...
        const importEntities = async (lines) => {
          const decodedLines = lines.map((line) => decode(Buffer.from(line, 'hex')))
          const knowns = []
          const unknowns = []
          for (let {attributes, id} of decodedLines) {
            ...
            if (known) {
              knowns.push({id, attributes, known})
            } else {
              unknowns.push({id, ...attributes})
            }
          }
          const newers = knowns.filter(({known, attributes}) => new Date(known.$$at) < new Date(attributes.$$at))
          if (newers.length > 0) {
            const attributesHash = newers.recude((hash, {id, attributes}) => ({ ...hash, [id]: attributes }), {})
        ...
        await new Promise((resolve, reject) => {
          ...
          let lines = []
          reader.on('line', (line) => {
            work = Promise.resolve(work).then(async (line) => {
              if (lines.length > 100) {
                lines.push(line)
              } else {
                await importEntities([...lines])
                lines = []
              }
            })
          })
