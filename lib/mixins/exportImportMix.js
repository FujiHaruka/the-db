    ...
    async import (dirname) {
      ...
      for (const [rName, rInfo] of Object.entries(resourcesInfo)) {
        ...
        const importEntities = async (lines) => {
          ...
          const newers = knowns.filter(({attributes, known}) => new Date(known.$$at) < new Date(attributes.$$at))
          if (newers.length > 0) {
            const attributesHash = newers.recude((hash, {attributes, id}) => ({...hash, [id]: attributes }), {})
