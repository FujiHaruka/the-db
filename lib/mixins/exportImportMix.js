  ...
  class ExportImportMixed extends Class {
    async export (dirname) {
      ...
      for (const resourceName of Object.keys(resources)) {
        ...
        const filename = path.join(dirname, resourceName + '.dat')
        await writeFileAsync(filename, '')
        logger.point(`Exporting "${resourceName}"...`)
        let length = 0
    ...
    async import (dirname) {
      ...
      for (const resourceName of Object.keys(resourcesInfo)) {
        const resource = s.resource(resourceName)
        logger.point(`Importing "${resourceName}"...`)
        const {filename} = resourcesInfo[resourceName]
        let work = null
        await new Promise((resolve, reject) => {
          ...
          reader.on('line', (line) => {
            work = Promise.resolve(work).then(async () => {
              ...
              if (known) {
                console.warn('Skip to known entity with id', id)
